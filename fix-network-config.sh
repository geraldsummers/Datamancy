...
# Datamancy Network Configuration Fix
# Fixes dual IP address conflict and TCP keepalive settings
...
# Backup interfaces file if not already backed up
...
# Check if interfaces file needs fixing
...
# eno1 is managed by systemd-networkd, not ifupdown
# See /etc/systemd/network/20-wired-static.network
EOF
...
if [ $NEEDS_SYSCTL_UPDATE -eq 1 ]; then
    # Remove old keepalive settings if present
...
# TCP keepalive for long-running connections (5 min check)
# Added by Datamancy network fix script
