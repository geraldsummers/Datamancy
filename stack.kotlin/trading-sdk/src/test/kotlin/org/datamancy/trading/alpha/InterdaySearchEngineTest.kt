...
class InterdaySearchEngineTest {
    ...
    fun `run attaches multiplicity evidence when comparison configs are supplied`() = kotlinx.coroutines.runBlocking {
        ...
        assertTrue(multiplicity.folds.isNotEmpty())
        assertTrue(multiplicity.folds.size <= 5)
        assertTrue(multiplicity.folds.all { it.atomicBlockBars > 0 })
        assertTrue(multiplicity.folds.all { it.activeBlockCount >= 1 })
        assertTrue(multiplicity.folds.all { it.purgeBlockCount >= 1 })
        assertTrue(multiplicity.folds.all { it.trainingObservationCount >= 0 })
        assertTrue(multiplicity.folds.all { it.empiricalWeightFitPasses >= 1 })
