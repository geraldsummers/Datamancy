...
fun main() {
    ...
    val bookstackWriterUrl = System.getenv("BOOKSTACK_WRITER_URL") ?: "http://data-bookstack-writer:8099"
    val vectorIndexerUrl = System.getenv("VECTOR_INDEXER_URL") ?: "http://data-vector-indexer:8100"
    ...
    val bookstackWriter = BookStackWriterClient(bookstackWriterUrl)
    val vectorIndexer = VectorIndexerClient(vectorIndexerUrl)

    // Initialize source adapter (ClickHouse is the single source of truth)
    val clickhouseAdapter = ClickHouseSourceAdapter(clickhouseUrl)

    // Initialize transformer
    val transformer = DataTransformer(
        database = database,
        sourceAdapter = clickhouseAdapter,
        bookstackWriter = bookstackWriter,
        vectorIndexer = vectorIndexer
    )

    val port = System.getenv("UNIFIED_INDEXER_PORT")?.toIntOrNull() ?: ServicePorts.DataTransformer.INTERNAL
    val server = embeddedServer(Netty, port = port) {
        configureServer(database, transformer)
    }
...
fun Application.configureServer(database: Database, transformer: DataTransformer) {
    ...
    routing {
        ...
        post("/index/collection/{collection}") {
            ...
            launch {
                try {
                    transformer.indexCollection(collection, fullReindex)
        ...
        post("/index/all") {
            ...
            collections.forEach { collection ->
                launch {
                    try {
                        transformer.indexCollection(collection, fullReindex = false)
                        application.log.info("Indexed collection: $collection")
                    } catch (e: Exception) {
                        application.log.error("Failed to index collection: $collection", e)
                    }
        ...
        get("/api/indexer/collections") {
            try {
                val adapter = transformer.javaClass.getDeclaredField("sourceAdapter").let { field ->
                    field.isAccessible = true
                    field.get(transformer)
                }
