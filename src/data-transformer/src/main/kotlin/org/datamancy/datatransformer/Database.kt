...
interface DatabaseApi {
    ...
    fun updateJobProgress(jobId: UUID, indexedPages: Int = 0, totalPages: Int = 0, currentPageId: Int? = null)
    fun completeJob(jobId: UUID, status: String, error: String?)
    fun markJobComplete(jobId: UUID)
    fun markJobFailed(jobId: UUID, errorMessage: String)
    fun incrementIndexedPages(jobId: UUID)
    fun recordError(jobId: UUID, pageId: Int, errorMessage: String)
    fun getContentHash(pageId: Int): String?
...
class Database(
    ...
    override fun markJobComplete(jobId: UUID) {
        completeJob(jobId, "completed", null)
    }

    override fun markJobFailed(jobId: UUID, errorMessage: String) {
        completeJob(jobId, "failed", errorMessage)
    }

    override fun incrementIndexedPages(jobId: UUID) {
        dataSource.connection.use { conn ->
            conn.prepareStatement("""
                UPDATE indexing_jobs
                SET indexed_pages = indexed_pages + 1
                WHERE job_id = ?
            """).use { stmt ->
                stmt.setObject(1, jobId)
                stmt.executeUpdate()
    ...
    override fun recordError(jobId: UUID, pageId: Int, errorMessage: String) {
        logError(jobId, pageId, null, errorMessage, "")
        dataSource.connection.use { conn ->
            conn.prepareStatement("""
                UPDATE indexing_jobs
    ...
    override fun getContentHash(pageId: Int): String? {
        dataSource.connection.use { conn ->
            conn.prepareStatement("""
                SELECT content_hash FROM indexed_pages WHERE page_id = ?
            """).use { stmt ->
                stmt.setInt(1, pageId)
                val rs = stmt.executeQuery()
                return if (rs.next()) rs.getString("content_hash") else null
            }
        }
    }
