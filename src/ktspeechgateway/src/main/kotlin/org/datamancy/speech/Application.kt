                ...
                try {
                    ...
                    val formData = formData {
                        append(
                            ...
                            value = audioBytes!!,
                            headers = headersOf(
                                HttpHeaders.ContentType, ContentType.Application.OctetStream.toString(),
                                HttpHeaders.ContentDisposition, "form-data; name=\"audio_file\"; filename=\"$fileName\""
                            )
                        )
                    }

                    val respText = submitFormWithBinaryData(
            ...
            post("/v1/audio/speech") {
                try {
                    ...
                    val upstreamResp: HttpResponse = client.post("$PIPER_BASE_URL/api/tts") {
                        ...
                        setBody(kotlinx.serialization.json.buildJsonObject { put("text", text) })
                    }
                    val audio = upstreamResp.bodyAsBytes()
                    call.respondBytes(bytes = audio, contentType = ContentType.Audio.WAV)
                } catch (e: Exception) {
                    call.respond(HttpStatusCode.InternalServerError, mapOf("error" to (e.message ?: "failed")))
                }
            }
