...
docker cp $(docker compose ps -q integration-test-runner):/app/test-results ~/test-results-$(date +%Y%m%d-%H%M%S)

# Or just screenshots
docker cp $(docker compose ps -q integration-test-runner):/app/test-results/screenshots ~/screenshots-$(date +%Y%m%d-%H%M%S)
```
...
docker cp $(docker compose ps -q integration-test-runner):/app/test-results ~/test-results
