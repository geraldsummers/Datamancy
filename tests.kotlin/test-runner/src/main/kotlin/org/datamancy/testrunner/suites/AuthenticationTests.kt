...
suspend fun TestRunner.authenticationTests() = suite("Authentication & Authorization Tests") {
    ...
    test("ldap-ensure-suffixes init container completed successfully") {
        val result = DockerCli.run(
            "inspect", "-f", "{{.State.Status}}|{{.State.ExitCode}}", "ldap-ensure-suffixes"
        )
        require(result.exitCode == 0) {
            "Unable to inspect ldap-ensure-suffixes container: ${result.output}"
        }
    ...
    test("Seafile requires authentication for API") {
        ...
        require(response.status in listOf(HttpStatusCode.OK, HttpStatusCode.Forbidden, HttpStatusCode.InternalServerError, HttpStatusCode.BadGateway)) {
        ...
        if (response.status == HttpStatusCode.InternalServerError) {
            ...
            println("      ℹ️  Seafile API returned 502 (upstream startup race; autoheal should recover)")
        } else {
