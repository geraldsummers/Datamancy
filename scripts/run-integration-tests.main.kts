...
fun buildTestRunner(envFile: Path) {
    info("Building test runner image...")
    run("docker", "compose", "--env-file", envFile.toString(), "--profile", "testing", "build", "integration-test-runner")
}

fun runTests(args: List<String>, envFile: Path): Int {
    info("Running integration tests inside Docker network...")
    println()

    val cmd = mutableListOf(
        "docker", "compose", "--env-file", envFile.toString(), "--profile", "testing",
...
buildTestRunner(envFile)

// Run integration tests
val testArgs = args.toList()
val exitCode = runTests(testArgs, envFile)
