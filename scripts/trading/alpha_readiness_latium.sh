...
if label == "data-health-summary":
    ...
    for exchange in payload.get("exchanges", [])[:5]:
...
readiness_json="$(remote_http_post "/api/v1/alpha/cross-sectional/readiness" "{\"marketExchange\":\"$exchange\",\"persistBacktest\":false,\"persistForward\":false,\"enablePaperOrders\":false}")"
...
print_json_summary "research-readiness" "$readiness_json"
...
if not readiness.get("passed", False):
    print("[verdict] BLOCKED")
    print(f"  - {readiness.get('reason')}")
    sys.exit(2)
