...
fun copyComposeFiles(
    ...
    // Merge all YAML files programmatically
    val yamlMapper = ObjectMapper(YAMLFactory()).registerModule(KotlinModule.Builder().build())
    val mergedServices = mutableMapOf<String, Any>()
    val mergedVolumes = mutableMapOf<String, Any>()

    // Parse and merge volume-init
    if (volumeInitFile.exists()) {
    ...
    // Parse networks
    val networks = settingsDir.resolve("networks.yml").let { file ->
        if (file.exists()) {
            val content = file.readText().let { substituteEnvironmentVariables(it, credentials, sanitized, config) }
            (yamlMapper.readValue(content, Map::class.java) as Map<*, *>)["networks"]
        } else null
    }

    // Parse and merge volumes from settings
    settingsDir.resolve("volumes.yml").let { file ->
        if (file.exists()) {
            val content = file.readText().let { substituteEnvironmentVariables(it, credentials, sanitized, config) }
    ...
    val finalMap = mutableMapOf<String, Any>("services" to mergedServices)
    if (networks != null) finalMap["networks"] = networks
    if (mergedVolumes.isNotEmpty()) finalMap["volumes"] = mergedVolumes

    // Write as YAML
    val mainCompose = buildString {
        appendLine("# Auto-generated docker-compose.yml with all services merged")
        appendLine("# Generated by build-datamancy-v2.main.kts at ${Instant.now()}")
        appendLine()
        append(yamlMapper.writeValueAsString(finalMap))
    }
