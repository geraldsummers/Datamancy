...
fun copyComposeFiles(
    ...
    // Create main docker-compose.yml by concatenating all templates with YAML merge keys
    val mainCompose = buildString {
        ...
        val allTemplates = mutableListOf<String>()

        if (volumeInitFile.exists()) {
            allTemplates.add(volumeInitFile.readText()
                .let { substituteEnvironmentVariables(it, credentials, sanitized, config) })
        }

        serviceFiles.forEach { file ->
            allTemplates.add(file.readText()
                .let { substituteEnvironmentVariables(it, credentials, sanitized, config) })
        }

        // Parse and merge using yq/docker compose's native merge
        // Just concatenate with --- document separators and let docker compose merge handle it
        allTemplates.forEachIndexed { index, content ->
            if (index > 0) appendLine("---")
            appendLine(content)
        }

        // Add networks and volumes at the end as final document
        appendLine("---")
