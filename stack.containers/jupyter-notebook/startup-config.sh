...
write_notebook(
    ...
    [
        ...
        code_cell(
            ...
            "LEFT JOIN fu ON fu.time = px.time\n"
            "ORDER BY px.time ASC\n"
            "''')\n"
            "df = pd.read_sql(\n"
            "    sql,\n"
            "    engine,\n"
            "    params={'exchange': exchange, 'symbol': symbol, 'lookback': lookback},\n"
            "    parse_dates=['time']\n"
            ")\n"
        ...
        code_cell(
            ...
            "df['spread_pct'] = np.where(df['spread_pct'] > 0, df['spread_pct'], ((df['best_ask'] - df['best_bid']) / np.where(df['mid_price'] == 0, np.nan, df['mid_price']))).astype(float)\n"
            ...
            "\n"
            "# Funding carry overlay (per-minute approximation from sparse funding rows)\n"
            "df['funding_rate'] = df['funding_rate'].fillna(0)\n"
            "df['carry_overlay'] = (-df['funding_rate']).clip(-0.0008, 0.0008) / (8 * 60)\n"
            "\n"
        ...
        code_cell(
            ...
            "slippage_bps = base_slippage_bps + volatility_slippage_mult * df['vol_30m'] + (df['spread_pct'].clip(lower=0) * 10000 * 0.35)\n"
...
write_notebook(
    ...
    [
        markdown_cell(
            ...
            "`swyftx`, `binance`, `bybit`, `coinbase`, `dydx`, and `aster`.\n\n"
            "**Important:** `hyperliquid` routes are live-execution capable. They are excluded by default\n"
            "unless you explicitly enable them via `INCLUDE_LIVE_HYPERLIQUID=true`.\n\n"
        ...
        code_cell(
            ...
            "include_live_hyperliquid = os.getenv('INCLUDE_LIVE_HYPERLIQUID', 'false').strip().lower() in {'1','true','yes','on'}\n"
            "default_exchanges = ['swyftx','binance','bybit','coinbase','dydx','aster']\n"
            "if include_live_hyperliquid:\n"
            "    default_exchanges.append('hyperliquid')\n"
            "\n"
            "if not tx_token:\n"
            "    print('Set TX_AUTH_TOKEN in your notebook env before placing orders.')\n"
            "    print('Example in notebook: %env TX_AUTH_TOKEN=<your-jwt>')\n"
            "if include_live_hyperliquid:\n"
            "    print('WARNING: INCLUDE_LIVE_HYPERLIQUID=true enables live Hyperliquid order path.')\n"
        ),
        code_cell(
            "def auth_headers():\n"
            "    if not tx_token:\n"
            ...
            "    if exchange == 'hyperliquid' and not include_live_hyperliquid:\n"
            "        raise RuntimeError('hyperliquid is excluded by default because it can execute live orders. Set INCLUDE_LIVE_HYPERLIQUID=true to override.')\n"
            "    payload = {\n"
            "        'symbol': symbol,\n"
            "        'side': side,\n"
            "        'type': order_type,\n"
...
write_notebook(
    ...
    [
        markdown_cell(
            ...
            "- optionally persists rows into `strategy_latency_metrics`, `strategy_execution_costs`, and `strategy_live_backtest_drift`\n"
            "- feeds Grafana directly via the same tables used by gateway telemetry\n"
        ),
        code_cell(
            "import os\n"
            "import json\n"
        ...
        code_cell(
            "strategy_name = 'tx_gateway_forward_mainnet_data'\n"
            "exchange = 'hyperliquid'\n"
            "symbol = 'BTC'\n"
            "lookback = '12 hours'\n"
            "decision_every_minutes = 5\n"
            "notional_usd = 7500.0\n"
            "fee_tier = 'retail'  # retail|pro|vip\n"
            "base_maker_bps = -1.0\n"
            "base_taker_bps = 4.0\n"
            "fee_tier_adjustment_bps = {'retail': 0.0, 'pro': -0.7, 'vip': -1.5}.get(fee_tier, 0.0)\n"
            "latency_ms_base = 95.0\n"
            "latency_ms_jitter = 40.0\n"
            "rng = np.random.default_rng(7)\n"
        ),
        code_cell(
            "forward = pd.read_sql(text('''\n"
            "WITH px AS (\n"
            ...
            "       COALESCE(fu.funding_rate, 0) AS funding_rate\n"
            "FROM px\n"
            "LEFT JOIN ob ON ob.time = px.time\n"
            "LEFT JOIN fu ON fu.time = px.time\n"
            "ORDER BY px.time ASC\n"
            "'''), engine, params={'exchange': exchange, 'symbol': symbol, 'lookback': lookback}, parse_dates=['time'])\n"
        ...
        code_cell(
            ...
            "\n"
            "spread_bps = (forward['spread_pct'].clip(lower=0) * 10000.0)\n"
            ...
            "funding_drift_bps = (-forward['funding_rate'].fillna(0) * 10000.0).clip(-5.0, 5.0)\n"
        ...
        code_cell(
            "persist_forward = False  # Set True to feed Grafana tables with this forward-test stream\n"
            ...
            "                'executionMode': 'forward_mainnet_data',\n"
            "                'status': 'SIMULATED',\n"
            "                'urgencyClass': 'normal',\n"
            ...
            "                'metadata': metadata\n"
            "            })\n"
            "    print(f'Persisted {len(rows)} forward-test rows into execution/drift telemetry tables.')\n"
            "else:\n"
            "    print('Persistence disabled (set persist_forward=True to feed Grafana).')\n"
        )
    ]
)
PY
