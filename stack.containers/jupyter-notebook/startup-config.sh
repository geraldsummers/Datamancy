...
write_notebook(
    ...
    [
        ...
        code_cell(
            ...
            "    ex = exchanges or allowed_exchanges_for_mode(execution_mode)\n"
            "    params = {\n"
            "        'symbol': symbol,\n"
            "        'side': side,\n"
            "        'exchanges': ','.join(ex)\n"
            ...
            "    return place_order(exchange='hyperliquid', symbol=symbol, side=side, order_type=order_type, size=size, price=price, execution_mode='testnet_live', reduce_only=reduce_only, post_only=post_only, urgency_class=urgency_class, max_slippage_bps=max_slippage_bps, cancel_after_ms=cancel_after_ms)\n"
        ),
        code_cell(
