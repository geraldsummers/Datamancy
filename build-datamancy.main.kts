...
fun generateComposeFiles(registry: ServiceRegistry, outputDir: File, allServices: Map<String, ServiceDefinition>) {
    ...
    val volumesYaml = buildString {
        ...
        appendLine("# Init container creates all directories before other services start")
        appendLine()

        appendLine("services:")
        appendLine("  volume-init:")
        appendLine("    image: alpine:latest")
        appendLine("    container_name: volume-init")
        appendLine("    restart: \"no\"")
        appendLine("    volumes:")
        appendLine("      - \${VOLUMES_ROOT}:/volumes")
        appendLine("    command: >")
        appendLine("      sh -c '")
        appendLine("      echo \"Creating volume directory structure...\" &&")
        allVolumes.sorted().forEachIndexed { index, volumeName ->
            // Use volume name directly as directory path (no transformation)
            val continuation = if (index < allVolumes.size - 1) " &&" else ""
            appendLine("      mkdir -p /volumes/$volumeName$continuation")
        }
        appendLine("      echo \"All volume directories created successfully!\"")
        appendLine("      '")
        appendLine()
    ...
    info("Generated ${allVolumes.size} volume definitions with init container")
...
fun generateServicesYaml(
    ...
    val yaml = buildString {
        ...
        services.forEach { (name, svc) ->
            ...
            // Add volume-init dependency if service uses volumes
            if (svc.volumes?.isNotEmpty() == true) {
                dependencies.add("volume-init")
            }
            ...
            if (dependencies.isNotEmpty()) {
                ...
                dependencies.forEach { dep ->
                    appendLine("      $dep:")
                    appendLine("        condition: service_completed_successfully")
                }
            }
...
fun generateOverrideTemplate(outputDir: File) {
    info("Generating docker-compose.override.yml.example")
    outputDir.resolve("docker-compose.override.yml.example").writeText("""
# Example override file for custom volume paths
# Copy to docker-compose.override.yml and edit as needed
# This file is automatically loaded by docker compose when present
#
# Example: Use SSD for qdrant vector database
# volumes:
#   datamancy_qdrant_data:
#     driver: local
#     driver_opts:
#       type: none
#       o: bind
#       device: /path/to/fast/ssd/qdrant/data
""".trimIndent())
    success("Generated docker-compose.override.yml.example")
}
...
Building deployment-ready distribution...
    ...
    val domain = config.get("runtime")?.get("domain")?.asText() ?: "example.com"
    val adminEmail = config.get("runtime")?.get("admin_email")?.asText() ?: "admin@$domain"
    val adminUser = config.get("runtime")?.get("admin_user")?.asText() ?: "admin"
    ...
    generateOverrideTemplate(distDir)
    copyGpuOverride(distDir)
    generateTestPortsOverlay(distDir, allServices)
    success("Generated compose files + test overlay + override template + GPU override")
